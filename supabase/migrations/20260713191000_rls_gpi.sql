...
create policy persona_interna_detalle_select on public.persona_interna_detalle
  for select using (
    ...
    or public.tiene_permiso('GPI_PERSONA_DETALLE_SELECT')
    or public.tiene_acceso_operativo_cac()
  );
create policy persona_interna_detalle_insert_gpi on public.persona_interna_detalle
  for insert with check (public.tiene_permiso('GPI_PERSONA_DETALLE_INSERT'));
create policy persona_interna_detalle_update_gpi on public.persona_interna_detalle
  for update using (public.tiene_permiso('GPI_PERSONA_DETALLE_UPDATE'))
  with check (public.tiene_permiso('GPI_PERSONA_DETALLE_UPDATE'));
...
create policy registro_biometrico_select_adm on public.registro_biometrico
  for select using (public.tiene_permiso('ADM_BIOMETRIA_SELECT'));
create policy registro_biometrico_select_gpi on public.registro_biometrico
  for select using (public.tiene_permiso('GPI_BIOMETRIA_SELECT'));
create policy registro_biometrico_select_cac on public.registro_biometrico
  for select using (public.tiene_permiso('CAC_EVENTO_SELECT'));
...
create policy registro_biometrico_insert_gpi on public.registro_biometrico
  for insert with check (public.tiene_permiso('GPI_BIOMETRIA_INSERT'));
create policy registro_biometrico_update_gpi on public.registro_biometrico
  for update using (public.tiene_permiso('GPI_BIOMETRIA_UPDATE'))
  with check (public.tiene_permiso('GPI_BIOMETRIA_UPDATE'));
