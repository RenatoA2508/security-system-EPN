...
create policy memorando_select on public.memorando
  for select using (
    ...
    or auth.tiene_permiso('GPE_MEMORANDO_SELECT')
    or auth.tiene_permiso('CAC_EVENTO_SELECT')
  );
create policy memorando_insert_gpe on public.memorando
  for insert with check (auth.tiene_permiso('GPE_MEMORANDO_INSERT'));
create policy memorando_update_gpe on public.memorando
  for update using (auth.tiene_permiso('GPE_MEMORANDO_UPDATE'))
  with check (auth.tiene_permiso('GPE_MEMORANDO_UPDATE'));

-- ===== persona_memorando =====
create policy persona_memorando_select on public.persona_memorando
  for select using (
    auth.tiene_permiso('ADM_MODULO_ACCEDER')
    or auth.tiene_permiso('GPE_PERSONA_MEMORANDO_SELECT')
    or auth.tiene_permiso('CAC_EVENTO_SELECT')
  );
create policy persona_memorando_insert_gpe on public.persona_memorando
  for insert with check (auth.tiene_permiso('GPE_PERSONA_MEMORANDO_INSERT'));
create policy persona_memorando_update_gpe on public.persona_memorando
  for update using (auth.tiene_permiso('GPE_PERSONA_MEMORANDO_UPDATE'))
  with check (auth.tiene_permiso('GPE_PERSONA_MEMORANDO_UPDATE'));
...
create policy autorizacion_visita_select on public.autorizacion_visita_diaria
  for select using (
    auth.tiene_permiso('ADM_MODULO_ACCEDER')
    or auth.tiene_permiso('GPE_AUTORIZACION_SELECT')
    or auth.tiene_permiso('CAC_AUTORIZACION_SELECT')
  );
create policy autorizacion_visita_insert_gpe on public.autorizacion_visita_diaria
  for insert with check (auth.tiene_permiso('GPE_AUTORIZACION_INSERT'));
create policy autorizacion_visita_insert_guardia on public.autorizacion_visita_diaria
  for insert with check (auth.tiene_permiso('CAC_AUTORIZACION_INSERT'));
create policy autorizacion_visita_update_gpe on public.autorizacion_visita_diaria
  for update using (auth.tiene_permiso('GPE_AUTORIZACION_UPDATE'))
  with check (auth.tiene_permiso('GPE_AUTORIZACION_UPDATE'));
create policy autorizacion_visita_update_guardia on public.autorizacion_visita_diaria
  for update using (auth.tiene_permiso('CAC_AUTORIZACION_UPDATE'))
  with check (auth.tiene_permiso('CAC_AUTORIZACION_UPDATE'));
