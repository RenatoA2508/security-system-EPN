...
create policy regla_acceso_select on public.regla_acceso
  for select using (
    ...
    or auth.tiene_permiso('CAC_REGLA_SELECT')
    or auth.tiene_permiso('CAC_VALIDACION_EJECUTAR')
  );
create policy regla_acceso_insert_cac on public.regla_acceso
  for insert with check (auth.tiene_permiso('CAC_REGLA_INSERT'));
create policy regla_acceso_update_cac on public.regla_acceso
  for update using (auth.tiene_permiso('CAC_REGLA_UPDATE'))
  with check (auth.tiene_permiso('CAC_REGLA_UPDATE'));
...
create policy evento_acceso_insert_guardia on public.evento_acceso
  for insert with check (auth.tiene_permiso('CAC_EVENTO_INSERT'));
...
create policy alerta_seguridad_select_amplio on public.alerta_seguridad
  for select using (
    auth.tiene_permiso('ADM_MODULO_ACCEDER')
    or (auth.tiene_permiso('CAC_ALERTA_SELECT') and auth.tiene_permiso('CAC_EVENTO_SELECT'))
  );
...
create policy alerta_seguridad_select_guardia on public.alerta_seguridad
  for select using (
    auth.tiene_permiso('CAC_ALERTA_SELECT')
    and auth.tiene_permiso('CAC_EVENTO_SELECT_PUNTO_ASIGNADO')
    and exists (
      select 1
        from public.evento_acceso ea
       where ea.id_evento = alerta_seguridad.id_evento
         and ea.id_punto_control in (select auth.puntos_control_asignados())
    )
  );

-- Solo el Supervisor CAC atiende alertas (§D4).
create policy alerta_seguridad_update_cac on public.alerta_seguridad
  for update using (auth.tiene_permiso('CAC_ALERTA_ATENDER'))
  with check (auth.tiene_permiso('CAC_ALERTA_ATENDER'));
