...
create policy zona_select on public.zona
  for select using (
    ...
    or public.tiene_permiso('PCO_ZONA_SELECT')
    or public.tiene_acceso_operativo_cac()
  );
create policy zona_insert_pco on public.zona
  for insert with check (public.tiene_permiso('PCO_ZONA_INSERT'));
create policy zona_update_pco on public.zona
  for update using (public.tiene_permiso('PCO_ZONA_UPDATE'))
  with check (public.tiene_permiso('PCO_ZONA_UPDATE'));

-- ===== punto_control =====
create policy punto_control_select on public.punto_control
  for select using (
    public.tiene_permiso('ADM_MODULO_ACCEDER')
    or public.tiene_permiso('PCO_PUNTO_CONTROL_SELECT')
    or public.tiene_acceso_operativo_cac()
  );
create policy punto_control_insert_pco on public.punto_control
  for insert with check (public.tiene_permiso('PCO_PUNTO_CONTROL_INSERT'));
create policy punto_control_update_pco on public.punto_control
  for update using (public.tiene_permiso('PCO_PUNTO_CONTROL_UPDATE'))
  with check (public.tiene_permiso('PCO_PUNTO_CONTROL_UPDATE'));
...
create policy dispositivo_select_amplio on public.dispositivo
  for select using (
    public.tiene_permiso('ADM_MODULO_ACCEDER')
    or public.tiene_permiso('PCO_DISPOSITIVO_SELECT')
...
create policy dispositivo_select_guardia on public.dispositivo
  for select using (
    ...
    and dispositivo.id_punto_control in (select public.puntos_control_asignados())
  );
create policy dispositivo_insert_pco on public.dispositivo
  for insert with check (public.tiene_permiso('PCO_DISPOSITIVO_INSERT'));
create policy dispositivo_update_pco on public.dispositivo
  for update using (public.tiene_permiso('PCO_DISPOSITIVO_UPDATE'))
  with check (public.tiene_permiso('PCO_DISPOSITIVO_UPDATE'));
...
create policy guardia_punto_control_select_amplio on public.guardia_punto_control
  for select using (
    public.tiene_permiso('ADM_MODULO_ACCEDER')
    or public.tiene_permiso('PCO_ASIGNACION_SELECT')
...
create policy guardia_punto_control_select_propia on public.guardia_punto_control
  for select using (
    public.tiene_permiso('CAC_ASIGNACION_SELECT_PROPIA')
...
create policy guardia_punto_control_insert_pco on public.guardia_punto_control
  for insert with check (public.tiene_permiso('PCO_ASIGNACION_INSERT'));
create policy guardia_punto_control_insert_cac on public.guardia_punto_control
  for insert with check (public.tiene_permiso('CAC_ASIGNACION_INSERT'));
create policy guardia_punto_control_update_pco on public.guardia_punto_control
  for update using (public.tiene_permiso('PCO_ASIGNACION_UPDATE'))
  with check (public.tiene_permiso('PCO_ASIGNACION_UPDATE'));
create policy guardia_punto_control_update_cac on public.guardia_punto_control
  for update using (public.tiene_permiso('CAC_ASIGNACION_UPDATE'))
  with check (public.tiene_permiso('CAC_ASIGNACION_UPDATE'));
